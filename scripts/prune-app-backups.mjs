#!/usr/bin/env node
// /Applications 里只应该有一个「Model Router」。历史上每次安装都会在同一个目录留下
...
export async function pruneAppBackups({ appPath, backupDir, keep = 2, dryRun = false }) {
  ...
  // 备份目录里存的是完整应用包：「Model Router-<时间戳>.app」，所以按去掉 .app 的名字匹配。
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const appPath = process.argv[2] || "/Applications/Model Router.app";
